...
class JWTCoderImpl @Inject()()(implicit val configuration: Configuration)
   extends JWTCoder {
   ...
   def getBasicCredentials(session: JwtSession) = {
      ...
      val username = session
         .getAs[String](TokenRep.USERNAME)
         .getOrElse(
            throw new RuntimeException("Can not get username from private token")
         )
      val secret: String = session
         .getAs[String](TokenRep.SECRET)
         .getOrElse(
            throw new RuntimeException("Can not get secret from private token")
         )
      val emailAddress: String = session
         .getAs[String](TokenRep.EMAIL)
         .getOrElse(
