...
trait UserService {
  ...
  def getUsersByEventId(eventId:Long)(implicit request: Request[_]) : Future[Seq[(User,UserDAO#TableElementType)]]
  ...
  def getFriends(usserId:Long)(implicit request: Request[_]):Future[Seq[(User,UserDAO#TableElementType)]]
  def getFriendsIds(userId:Long)(implicit request: Request[_]):Future[Seq[Long]]
  def findUser(userId:Long,query:String)(implicit request: Request[_]):Future[Seq[(User,UserDAO#TableElementType)]]
