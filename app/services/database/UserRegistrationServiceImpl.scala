...
class UserRegistrationServiceImpl @Inject()(
                                              ...
                                              private val userDAO: UserDAO[Future])(implicit ec: ExecutionContext)
   ...
   def confirmRegistrationAndCreateUser(registrationToken: String) = {
      val registrationTokenRep =
         jwtCoder.decodeRegistrationToken(registrationToken)
      val userRegistration = UserRegistration(
         username = registrationTokenRep.username,
         registrationToken = registrationToken)
      val tokenGen = jwtCoder.fromLazyUserId(registrationTokenRep)(_)
      userRegistrationDAO
         .confirmRegistrationAndGetUser(userRegistration, tokenGen)
         .map {
