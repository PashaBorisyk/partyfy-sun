...
private[sql] class UserToUserRelationTable(tag: Tag)
   extends Table[UserToUserRelation](tag, "user_to_user_relation") {
   ...
   def * =
      (userFrom, userTo, relation) <> (UserToUserRelation.tupled, UserToUserRelation.unapply)
