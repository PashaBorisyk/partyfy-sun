...
class EventServiceImpl @Inject()(@Named("kafka-producer") eventPublisher:ActorRef,
   ...
   override def create(eventWithUsersIds: (Event, Array[Int]))(implicit token:TokenRepPrivate) = {
      ...
      createAction.onComplete{ eventId =>
         if(eventId.getOrElse(0) != 0)
            eventPublisher ! EventCreatedRecord(token.userId,token.username,eventId.get,usersIds)
      }
      createAction
   }
   ...
   override def update(event: Event)(implicit token:TokenRepPrivate)  = {
      ...
      updateAction.onComplete{ updatedRows =>
         if(updatedRows.getOrElse(0) != 0)
            eventPublisher ! EventUpdatedRecord(token.userId,token.username,event.id)
      }
      updateAction
   }

   override def getEventsByOwner(userId: Int)(implicit token:TokenRepPrivate)  = {
      eventDAO.getEventsByOwner(userId)
   }

   override def getEventsByMemberId(userId: Int)(implicit token:TokenRepPrivate)  = {
      eventDAO.getEventsByMemberId(userId)
   }

   override def getEventIdsByMemberId(userId: Int)(implicit token:TokenRepPrivate) = {
   ...
   override def addUserToEvent(eventId: Long, userId: Int)(implicit token:TokenRepPrivate)  = {
      ...
      addUserAction.onComplete{ insertedRows =>
         if(insertedRows.getOrElse(0) != 0){
            eventPublisher ! EventUserAddedRecord(token.userId,token.username,eventId,userId)
         }
      }
      addUserAction
   }
   ...
   override def removeUserFromEvent(eventId: Long,userId: Int)(implicit token:TokenRepPrivate)  = {
      val removeUserAction = eventDAO.removeUserFromEvent(eventId,token.userId)
      removeUserAction.onComplete{ deletedRows =>
         if(deletedRows.getOrElse(0) != 0){
            eventPublisher ! EventUserRemovedRecord(token.userId,token.username,eventId,userId)
         }
      }
      removeUserAction
   }
   ...
   private final def writeEventFields(event:Event)(implicit token: TokenRepPrivate) ={
